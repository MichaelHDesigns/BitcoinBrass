...
CzBRSTracker::CzBRSTracker(std::string strWalletFile)
{
...
CzBRSTracker::~CzBRSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBRSTracker::Init()
{
...
bool CzBRSTracker::Archive(CMintMeta& meta)
{
...
bool CzBRSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBRSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBRSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBRSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBRSTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBRSTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBRSTracker::GetSerialHashes()
{
...
CAmount CzBRSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBRSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBRSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBRSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBRSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBRSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBRSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBRSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBRSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBRSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBRSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBRSWallet* zBRSWallet)
{
    bool iszBRSWalletInitialized = (NULL != zBRSWallet);
    CMintMeta meta;
    ...
    if (! iszBRSWalletInitialized)
        zBRSWallet = new CzBRSWallet(strWalletFile);
    meta.isSeedCorrect = zBRSWallet->CheckSeed(dMint);
    if (! iszBRSWalletInitialized)
        delete zBRSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBRSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBRSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBRSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBRSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBRSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBRSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBRSWallet* zBRSWallet = new CzBRSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBRSWallet);
        }
        delete zBRSWallet;
        LogPrint("zero", "%s: added %d dzbrs from DB\n", __func__, listDeterministicDB.size());
    }
