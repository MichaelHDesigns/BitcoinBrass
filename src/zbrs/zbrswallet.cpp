...
CzBRSWallet::CzBRSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbrs seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBRSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBRSSeed_deprecated()) {
                    LogPrintf("%s: Updated zBRS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbrs seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbrs wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBRSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBRSCount(nCountLastUsed);
    else if (!walletdb.ReadZBRSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBRSWallet::Lock()
{
    seedMaster = 0;
}

void CzBRSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBRSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBRS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBRSWallet::LoadMintPoolFromDB()
{
...
void CzBRSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBRSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBRSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbrsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBRSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBRS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbrsTracker which also adds to database
    pwalletMain->zbrsTracker->Add(dMint, true);
...
void CzBRSWallet::SeedToZBRS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBRSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBRSWallet::GenerateDeterministicZBRS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBRSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBRSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBRSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
