...
/**
 * BitcoinBrass RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZbrsSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzbrsseed(const UniValue& params, bool fHelp);
extern UniValue getzbrsseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzbrs(const UniValue& params, bool fHelp);
extern UniValue dzbrsstate(const UniValue& params, bool fHelp);
