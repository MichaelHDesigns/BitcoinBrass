...
#define URI_SCHEME "bitcoinbrass"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitcoinBrass address (e.g. %1)").arg("B7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no BitcoinBrass: URI
...
{
    // Convert bitcoinbrass:// to bitcoinbrass:
    //
    //    Cannot handle this later, because bitcoinbrass:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BRS, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open bitcoinbrass.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinBrass.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BitcoinBrass.lnk
...
{
    return GetAutostartDir() / "bitcoinbrass.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a bitcoinbrass.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BitcoinBrass\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the bitcoinbrass app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add bitcoinbrass app to startup item list
