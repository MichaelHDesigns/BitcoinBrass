...
#include "ui_zbrscontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbrsTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbrsTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBRS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBRS seed used to mint this zBRS is not the same as currently hold in the wallet";
            else
