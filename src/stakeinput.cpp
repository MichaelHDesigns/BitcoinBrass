...
// The zBRS block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBRS is a hash of the serial
...
{
    //Create an output returning the zBRS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBRSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBRS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBRS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBRSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBRS output", __func__);
...
{
    CzBRSTracker* zbrsTracker = pwallet->zbrsTracker.get();
    CMintMeta meta;
    if (!zbrsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbrsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BRS stake is the outpoint
