...
public:
    ...
    bool WriteZBRSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBRSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBRSSeed_deprecated(uint256& seed);
    bool EraseZBRSSeed();
    bool EraseZBRSSeed_deprecated();

    bool WriteZBRSCount(const uint32_t& nCount);
    bool ReadZBRSCount(uint32_t& nCount);
